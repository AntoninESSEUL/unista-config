...
export const validateRobotVarIndexForRobot = (
  ...
  if (!isRobotSelected(mask, robotIndex)) return { hasError: false, message: "" };

  const val = param.robotVarIndex?.[robotIndex];
  if (val == null || val <= 0) return { hasError: true, message: "Index required (> 0)" };
...
export const hasRobotVarIndexError = (param: models.Parameter, allParameters: models.Parameter[]): boolean => {
  const mask = parseRobotMask(param.robotMask);
  if (mask === 0) return false;

  for (let r = 0; r < 32; r++) {
    if (!isRobotSelected(mask, r)) continue;
    if (validateRobotVarIndexForRobot(param, r, allParameters).hasError) return true;
  }
  return false;
};
...
export const isConfigFieldVisibleCore = (
  ...
  switch (equipmentType) {
    case "camera": {
      ...
      const isBrandCompatible = brand === "CAMERA_UNDEFINED" || (brand?.includes("Keyence") ?? false);
    ...
    case "axis": {
      ...
      switch (field) {
        ...
        case "homingBackoffDistance":
          return (
            driveReference !== "P76" &&
            (homingType === "ABSOLUTE_SWITCH" || homingType === "BLOCK_TORQUE" || homingType === "BLOCK_LAG_ERROR")
          );
...
export const allFieldDefs = (definition: EquipmentDefinition): ConfigField[] => [
  ...
  ...(definition.limitsFields || []),
  ...(definition.defaultValuesFields || []),
  ...(definition.reductionFields || []),
  ...(definition.homingFields || []),
  ...(definition.autoTuneFields || []),
  ...(definition.inverterFields || []),
];
...
export async function buildValidationCaches(
  ...
  for (const eq of list) {
    ...
    for (const field of allFields) {
      fieldVisibility[eq.id][field.field] = isConfigFieldVisibleCore(eq.type, eq, field.field);
    }
  }
...
export const isFieldVisible = (eq: EquipmentInstance, cfg: ConfigField, caches: ValidationCaches): boolean => {
...
export const isNameError = (eq: EquipmentInstance, allEquipment: EquipmentInstance[]): boolean => {
  if (!eq.enable) return false;
...
export const isFieldError = (
  ...
  if (cfg && (cfg.type === "select" || cfg.type === "number") && (val === null || val === undefined || val === "")) {
  ...
  if (field === "jobId" && definition.type === "workplace") {
    ...
    if (eq.robotId) {
      const duplicate = workplaceList.find((j) => j.id !== eq.id && j.robotId === eq.robotId && j.jobId === eq.jobId);
  ...
  if (definition.type === "axis") {
    ...
    if (field === "defaultVelocity" || field === "defaultAcceleration" || field === "defaultDeceleration") {
  ...
  if (field === "ipAddress") {
    ...
    return allEquipment.some(
      (e) => e.enable && e.id !== eq.id && e.ipAddress !== undefined && e.ipAddress === eq.ipAddress,
    );
  }
  return false;
};

export const getParamErrorMessage = (param: models.Parameter, caches: ValidationCaches): string | null =>
  caches.paramErrorMessages[param.id] ?? null;
...
export const hasParamsError = (eq: EquipmentInstance, caches: ValidationCaches): boolean => {
  ...
  return params.some((p: models.Parameter) => getParamErrorMessage(p, caches) !== null);
};
...
export const hasLocalError = (
  ...
  if (!eq.enable) return false;
  if (isParentLinkBroken(eq, definition, moduleList, robots) || isNameError(eq, allEquipment)) return true;
...
export const getErrorMessage = (
  ...
  if (isParentLinkBroken(eq, definition, moduleList, robots)) {
    return definition.type === "workplace" ? "Robot Disabled or Missing" : "Module Disabled or Missing";
  }
  ...
  if (definition.type === "axis") {
    ...
    if (refs.length === 0) return "No compatible hardware found in the configuration.";
    if (!eq.hardwareReference || eq.hardwareReference === "") return 'Please select a value for "Hardware Reference"';

    if (
      eq.nodeNumber !== null &&
      eq.nodeNumber !== undefined &&
      eq.nodeNumber !== "" &&
      isFieldError(eq, "nodeNumber", definition, caches, allEquipment, workplaceList)
    ) {
  ...
  const missingField = allFieldDefs(definition).find(
    (f) =>
      ...
      (eq[f.field] === null || eq[f.field] === undefined || eq[f.field] === "") &&
      isFieldVisible(eq, f, caches),
  );
  if (missingField) return `Please select a value for "${missingField.label}"`;

  const params = (eq.parameters as models.Parameter[]) ?? [];
  const badParam = params.find((p: models.Parameter) => getParamErrorMessage(p, caches) !== null);
...
export const listHasErrors = (
  ...
  allEquipment: EquipmentInstance[],
  workplaceList: EquipmentInstance[],
): boolean => list.some((eq) => hasLocalError(eq, definition, caches, moduleList, robots, allEquipment, workplaceList));
