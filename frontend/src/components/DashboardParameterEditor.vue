<template>
  <div class="pt-4 pb-8 h-full flex flex-col w-full overflow-hidden relative bg-gray-50/50">
    <div class="flex flex-row gap-5 overflow-x-auto pb-8 pt-2 px-5 flex-1 items-start">
      <DashboardCard v-for="(entity, idx) in entities" :key="entity.id" :widthClass="widthClass" :bodyClass="bodyClass">
        <template #header>
          <div class="flex flex-col pointer-events-none max-w-[200px]">
            <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">
              {{ typeof headerSubtitle === "function" ? headerSubtitle(entity) : headerSubtitle }}
            </span>
            <div class="h-5 flex items-center mb-1">
              <span class="text-base font-bold text-gray-900 font-mono leading-none">
                {{ typeof headerTitle === "function" ? headerTitle(entity) : entity.name }}
              </span>
            </div>
            <p v-if="entity.detail" class="text-[10px] text-gray-400 leading-normal font-medium italic">
              {{ entity.detail }}
            </p>
          </div>
        </template>
...
const props = withDefaults(
  defineProps<{
    entities: Array<{ id: string; name: string; ui: any } & Record<string, any>>;
...
const {
  ...
  (entityIdx: number, paramIdx: number) => {
...
const sidebarContext = computed(() => {
  ...
  const entityIdx = selectedArgs.value[0]; // <-- CHANGEMENT ICI
  return `${props.contextLabel}: ${props.entities[entityIdx]?.name ?? ""}`;
});
