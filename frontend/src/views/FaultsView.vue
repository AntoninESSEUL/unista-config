<template>
  <div class="pt-4 pb-8 h-full flex flex-col w-full overflow-hidden relative bg-gray-50/50">
    <div class="flex flex-row gap-5 overflow-auto pb-8 pt-2 px-5 flex-1 items-start">
      <div
      ...
      >
        <div class="px-5 py-3.5 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between shrink-0">
          <div class="flex flex-col pointer-events-none">
            <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Configuration</span>
            <div class="h-5 flex items-center">
              <span class="text-base font-bold text-gray-900 font-mono leading-none">{{ group.name }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 flex flex-col gap-3">
          ...
          >
            ...
            <ParameterGrid :params="cat.items" :cols="1" @open="openSidebar(groupIdx, catIdx, $event)" />
          </CollapsibleSection>
        </div>
      </div>
    </div>
...
import { countActive, faultGroups } from "../core";
...
/**
 * Destructure generic selection composable methods and state.
 */
const {
  args: selectedArgs, // <-- CHANGEMENT ICI
  activeParam,
  open: openSidebar,
  close: closeSidebar,
} = useParamSelection((gIdx: number, cIdx: number, pIdx: number) => {
...
const activeContext = computed(() => {
  if (!selectedArgs.value) return "";

  const [gIdx, cIdx] = selectedArgs.value; // <-- CHANGEMENT ICI
  const group = faultGroups.value[gIdx];
  if (!group) return "";
