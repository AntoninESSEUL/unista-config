// backend/services/hardware_parser_service.go
package services
...
// Pre-compile the regex once at startup to avoid CPU overhead during XML parsing.
var channelRegex = regexp.MustCompile(`Channel\[(\d+)\]`)

// HardwareParserService provides methods to parse and classify B&R hardware files.
type HardwareParserService struct{}

// NewHardwareParserService creates a new instance of the parser service.
func NewHardwareParserService() *HardwareParserService {
	return &HardwareParserService{}
}

// xmlParameter represents the B&R <Parameter> tag.
...
// xmlModule represents the <Module> tag and its attributes.
type xmlModule struct {
	Name        string         `xml:"Name,attr"`
	Type        string         `xml:"Type,attr"`
	Description string         `xml:"Description,attr"`
	NodeNumber  int            `xml:"NodeNumber,attr"`
	Parameters  []xmlParameter `xml:"Parameter"` // Automatically parses direct children
}

// ParseHardwareFile reads and decodes the standard Hardware.hw XML file.
...
// classifyModule determines the category and axes count based on the hardware type.
func (s *HardwareParserService) classifyModule(mod xmlModule) (models.HardwareModule, bool) {
	hw := models.HardwareModule{
		Name:        mod.Name,
		ModuleType:  mod.Type,
		Description: mod.Description,
		NodeNumber:  mod.NodeNumber, // Direct retrieval of the NodeNumber
		AxesCount:   1,
	}

	// Precise extraction of channels from B&R parameters using the pre-compiled regex.
